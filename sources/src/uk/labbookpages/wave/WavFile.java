...
{
    // IGOR HACK! : all protected to protected
	protected enum IOState {READING, WRITING, CLOSED};
	protected final static int BUFFER_SIZE = 4096;

	protected final static int FMT_CHUNK_ID = 0x20746D66;
	protected final static int DATA_CHUNK_ID = 0x61746164;
	protected final static int RIFF_CHUNK_ID = 0x46464952;
	protected final static int RIFF_TYPE_ID = 0x45564157;

	protected File file;						// File that will be read from or written to
	protected IOState ioState;				// Specifies the IO State of the Wav File (used for snaity checking)
	protected int bytesPerSample;			// Number of bytes required to store a single sample
	protected long numFrames;					// Number of frames within the data section
	protected FileOutputStream oStream;	// Output stream used for writting data
	protected InputStream iStream;		// Input stream used for reading data   // IGOR HACK FileInputStream to InputStream
	protected double floatScale;				// Scaling factor used for int <-> float conversion				
	protected double floatOffset;			// Offset factor used for int <-> float conversion				
	...
	protected int blockAlign;					// 2 bytes unsigned, 0x0001 (1) to 0xFFFF (65,535)
	protected int validBits;					// 2 bytes unsigned, 0x0002 (2) to 0xFFFF (65,535)

	// Buffering
	protected byte[] buffer;					// Local buffer used for IO
	protected int bufferPointer;				// Points to the current position in local buffer
	protected int bytesRead;					// Bytes read after last read into local buffer
	...
	protected WavFile() // IGOR hack protected to protected
	{
	...
	// ------------------------------------------------
	protected static long getLE(byte[] buffer, int pos, int numBytes)
	{
		numBytes --;
		pos += numBytes;
	...
	protected static void putLE(long val, byte[] buffer, int pos, int numBytes)
	{
	...
	// --------------------------
	protected void writeSample(long val) throws IOException
	{
	...
	protected long readSample() throws IOException, WavFileException
	{
		long val = 0;
